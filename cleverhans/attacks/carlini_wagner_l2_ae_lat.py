...
class CWL2(object):
  def __init__(self, sess, model,cl_model, batch_size, confidence, targeted,
    ...
    x_hat_lat_logits = self.x_hat_lat
    x_hat_lat_logits = tf.clip_by_value(x_hat_lat_logits, epsilon, 1 - epsilon)
    x_hat_lat_logits = tf.log(x_hat_lat_logits / (1 - x_hat_lat_logits))
    '''
    ...
    #loss1 = 28* 28 * 1 * tf.reduce_mean(
     #       tf.nn.sigmoid_cross_entropy_with_logits(logits = slim.flatten(x_hat_lat_logits), labels = slim.flatten(targimg_lat)),
      #      reduction_indices=-1)
