...
class Attack(object):
  ...
  def construct_graph(self, fixed, feedable, x_val, x_targ_val, hash_key):
    """
    ...
    x_targ = tf.placeholder(self.tf_dtype, shape = x_shape)
    # now we generate the graph that we want
    x_adv = self.generate(x,x_targ, **new_kwargs)

    self.graphs[hash_key] = (x, x_targ, new_kwargs, x_adv)
  ...
  def generate_np(self, x_val,x_targ_val, **kwargs):
    """
    ...
    if hash_key not in self.graphs:
      self.construct_graph(fixed, feedable, x_val, x_targ_val, hash_key)
    else:
      # remove the None arguments, they are just left blank
      for k in list(feedable.keys()):
        if feedable[k] is None:
    ...
    x, x_t, new_kwargs, x_adv = self.graphs[hash_key]

    feed_dict = {x: x_val, x_t: x_targ_val}
