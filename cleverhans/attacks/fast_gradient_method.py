...
class FastGradientMethodAe(Attack):
  ...
  def __init__(self, model, sess=None, dtypestr='float32', **kwargs):
    ...
    super(FastGradientMethodAe, self).__init__(model, sess, dtypestr, **kwargs)
    self.feedable_kwargs = ('eps', 'clip_min', 'clip_max')
    self.structural_kwargs = ['ord', 'sanity_checks']

  def generate(self, x, x_target, **kwargs):
...
def fgm(x,
  ...
  #loss = tf.reduce_sum(tf.squared_difference(latent_orig, latent_target))/tf.to_float(tf.shape(x)[0] * 28*28)
  loss = tf.reduce_sum(tf.squared_difference(recon, x_target))/tf.to_float(tf.shape(x_target)[0])
  #loss = SquaredError(x, x_target)
  #if targeted:
  #loss = -loss
